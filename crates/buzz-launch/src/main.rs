...
use llull_buzz_launch::{admitted_mcp, checked_command, AcpGuard, ImageIdentity, Program, WORK};
...
async fn line<R: AsyncBufRead + Unpin>(
    ...
    loop {
        ...
        if buffer.is_empty() {
            return Err("upstream EOF before response".into());
        }
        let count = buffer
            .iter()
            .position(|b| *b == b'\n')
...
async fn exchange(
    ...
    let bytes = guard.admit(&canonical(
    ...
    input.flush().await?;
    let response: Value =
        parse(&tokio::time::timeout(Duration::from_secs(15), line(output)).await??)?;
    if response.get("id") != Some(&json!(id)) || response.get("error").is_some() {
        return Err("upstream protocol incompatibility; retain response locally for review".into());
    }
    response
        .get("result")
        .cloned()
        .ok_or_else(|| "missing upstream result".into())
}
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    if std::env::args().skip(1).collect::<Vec<_>>() != ["probe"] {
        return Err("only `probe` is available; live restricted activation is unavailable".into());
    }
    let identity = ImageIdentity::load()?;
    identity.verify()?;
    ...
    for entry in environment.split(|b| *b == 0).filter(|b| !b.is_empty()) {
        ...
        let (key, value) = entry.split_once('=').ok_or("invalid child environment")?;
        actual.insert(key.to_string(), value.to_string());
    }
    let expected: std::collections::BTreeMap<String, String> = llull_buzz_launch::environment()
        .into_iter()
    ...
    if actual != expected {
        ...
        return Err("actual child environment differs from closed launch policy".into());
    }
    ...
    let status = tokio::time::timeout(
        ...
        checked_command(&identity, Program::AcpHelp)?.status(),
    )
    .await??;
    if !status.success() {
        return Err("pinned buzz-acp CLI probe failed".into());
    }
    println!("{{\"probe\":\"ACP2-session-MCP\",\"inheritance_checked\":true,\"model_calls\":0,\"restricted_profile_active\":false}}");
    Ok(())
}
