...
pub fn environment() -> BTreeMap<&'static str, &'static str> {
    BTreeMap::from([
        ...
        ("BUZZ_AGENT_MAX_OUTPUT_TOKENS", "1"),
        ("BUZZ_AGENT_MAX_CONTEXT_TOKENS", "32000"),
        ("BUZZ_AGENT_MAX_LINE_BYTES", "1048576"),
        ("BUZZ_AGENT_MAX_HISTORY_BYTES", "16777216"),
        ("BUZZ_AGENT_MAX_PARALLEL_TOOLS", "1"),
        ("BUZZ_AGENT_MAX_HANDOFFS", "0"),
        ("BUZZ_AGENT_LLM_TIMEOUT_SECS", "1"),
        ("BUZZ_AGENT_TOOL_TIMEOUT_SECS", "5"),
        ("BUZZ_AGENT_MCP_INIT_TIMEOUT_SECS", "5"),
        ("BUZZ_AGENT_MCP_RESTART_MAX_ATTEMPTS", "1"),
...
impl ImageIdentity {
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        for directory in [WORK, HOME, TMP] {
            ...
            if std::fs::canonicalize(path).map_err(|_| Fault::Unavailable)? != path
                || !path.is_dir()
            {
                return Err(Fault::Denied);
            }
...
pub fn checked_command(
    ...
    identity.verify()?;
    Ok(fixed_command(program))
}

#[cfg(unix)]
fn immutable_file(path: &Path, executable: bool) -> Result<()> {
...
#[cfg(not(unix))]
fn immutable_file(_path: &Path, _executable: bool) -> Result<()> {
    Err(Fault::Unavailable)
}
...
pub fn admitted_mcp() -> McpServer {
    McpServer {
        name: "llull-foundation-probe".into(),
        command: BRIDGE.into(),
        args: vec![],
        env: vec![],
    }
...
impl AcpGuard {
    pub fn admit(&mut self, bytes: &[u8]) -> Result<Vec<u8>> {
        ...
        let params = match frame.method.as_str() {
            "initialize" => {
                if self.initialized
                    || frame.params
                        != json!({"protocolVersion":2,"clientCapabilities":{},"clientInfo":{"name":"llull-foundation-probe","version":"0.1.0"}})
                {
                    return Err(Fault::Denied);
                }
            ...
            "session/new" => {
                if !self.initialized || self.created {
                    return Err(Fault::Denied);
                }
                let requested: SessionNew =
                    serde_json::from_value(frame.params).map_err(|_| Fault::Invalid)?;
                if requested.cwd != Path::new(WORK) || requested.mcp_servers != vec![admitted_mcp()]
                {
                    return Err(Fault::Denied);
                }
                self.created = true;
    ...
    pub fn record_session(&mut self, session_id: &str) -> Result<()> {
        ...
        if !self.created || self.session.is_some() {
...
mod tests {
    ...
    fn frame(method: &str, params: Value) -> Vec<u8> {
        canonical(&json!({"jsonrpc":"2.0","id":1,"method":method,"params":params})).unwrap()
    }
    fn initialized() -> AcpGuard {
        let mut guard = AcpGuard::default();
        guard.admit(&frame("initialize",json!({"protocolVersion":2,"clientCapabilities":{},"clientInfo":{"name":"llull-foundation-probe","version":"0.1.0"}}))).unwrap();
        guard
    }
    #[test]
    fn authorized_probe_session_is_admitted_but_prompt_never_is() {
    ...
    #[test]
    fn wire_cannot_override_mcp_command_arguments_environment_or_cwd() {
        for mutation in 0..5 {
            let mut server = admitted_mcp();
            let mut cwd = WORK;
            match mutation {
                0 => server.command = "/bin/sh".into(),
                1 => server.args.push("-c".into()),
                2 => server.env.push(EnvVar {
                    name: "LD_PRELOAD".into(),
                    value: "/tmp/attack.so".into(),
                }),
                3 => server.env.push(EnvVar {
        ...
        for method in [
            ...
            assert_eq!(
                initialized().admit(&frame(method, json!({}))),
                Err(Fault::Unavailable)
            );
        }
    }
    #[test]
    fn inherited_dangerous_keys_are_not_in_launch_declarations() {
        ...
        assert_eq!(command.as_std().get_program(), AGENT);
        assert_eq!(command.as_std().get_current_dir(), Some(Path::new(WORK)));
    }
}
