...
use llull_buzz_wire::{digest, Command, Fault, Receipt, Admission, Execution, CONTRACT, PROFILE};
...
pub enum ProviderError {
    #[error(transparent)] Admission(#[from] Fault),
    #[error("provider persistence unavailable")] Storage(#[from] sqlx::Error),
    #[error("provider migration failed")] Migration(#[from] sqlx::migrate::MigrateError),
}
pub type Result<T> = std::result::Result<T, ProviderError>;
pub(crate) type Tx = Transaction<'static, Postgres>;

#[derive(Clone)]
pub struct Provider { pub(crate) pool: PgPool, public_origin: String, pub(crate) external_epoch: u64 }
...
impl Provider {
    pub fn new(pool: PgPool, public_origin: &str, external_epoch: u64) -> Result<Self> {
        if external_epoch == 0 || external_epoch > llull_buzz_wire::MAX_SAFE_INTEGER as u64 { return Err(Fault::Invalid.into()); }
        auth::https_origin(public_origin)?;
        Ok(Self { pool, public_origin: public_origin.trim_end_matches('/').into(), external_epoch })
    }
    pub async fn migrate(&self) -> Result<()> { sqlx::migrate!("../../migrations").run(&self.pool).await?; Ok(()) }
    ...
    pub async fn register(&self, registration: Registration, expected_digest: Option<&str>) -> Result<String> {
        ...
        match existing {
            None => {
                if expected_digest.is_some() || registration.revision != 1 { return Err(Fault::Conflict.into()); }
            ...
            Some(row) => {
                ...
                if expected_digest != Some(old.as_str()) || registration.revision != revision as u64 + 1 { return Err(Fault::Conflict.into()); }
                ...
                self.fence_consumer(&mut tx, &registration.consumer_id).await?;
            }
        }
        ...
        tx.commit().await?; Ok(fingerprint)
    }
    ...
    pub async fn set_service_active(&self, consumer:&str, active:bool, expected_digest:&str) -> Result<()> {
        let mut tx=self.begin().await?;
        ...
        if row.try_get::<String,_>("registration_sha256")?!=expected_digest || row.try_get::<bool,_>("active")?==active {return Err(Fault::Conflict.into());}
        let mut config=row.try_get::<sqlx::types::Json<Registration>,_>("registration")?.0;
        config.revision=config.revision.checked_add(1).ok_or(Fault::Exhausted)?;
        config.authority_epoch=config.authority_epoch.checked_add(1).ok_or(Fault::Exhausted)?;config.validate()?;
    ...
    pub(crate) async fn begin(&self) -> Result<Tx> {
        ...
        sqlx::query("SELECT recovery_epoch FROM provider_control WHERE singleton=1 FOR UPDATE").fetch_one(&mut *tx).await?;
        Ok(tx)
    }
    pub(crate) async fn now(tx: &mut Tx) -> Result<DateTime<Utc>> {
        Ok(sqlx::query_scalar("SELECT clock_timestamp()").fetch_one(&mut **tx).await?)
    }
    pub(crate) async fn epoch(tx: &mut Tx) -> Result<u64> {
        let epoch: i64 = sqlx::query_scalar("SELECT recovery_epoch FROM provider_control WHERE singleton=1").fetch_one(&mut **tx).await?;
        Ok(epoch as u64)
    }
    pub(crate) async fn authorize(&self, tx: &mut Tx, headers: &Headers<'_>, purpose: &str, path: &str, method: &str, body: &[u8], target: &Target<'_>) -> Result<(Registration, Claims)> {
        if !path.starts_with('/') || path.contains('?') || path.contains('#') { return Err(Fault::Invalid.into()); }
        let row = sqlx::query("SELECT registration, active FROM consumer_registry WHERE consumer_id=$1 FOR UPDATE")
            .bind(target.consumer).fetch_optional(&mut **tx).await?.ok_or(Fault::Denied)?;
        if !row.try_get::<bool,_>("active")? { return Err(Fault::Denied.into()); }
        let registration: Registration = row.try_get::<sqlx::types::Json<Registration>,_>("registration")?.0;
        let now = Self::now(tx).await?;
        let verified = auth::verify(&registration, headers, purpose, &format!("{}{path}", self.public_origin), method, body, target, now.timestamp())?;
        if verified.claims.recovery_epoch != Self::epoch(tx).await? || verified.claims.recovery_epoch != self.external_epoch { return Err(Fault::Denied.into()); }
        {
            let guard = TransactionReplayGuard { connection: Mutex::new(&mut **tx) };
            let scope = digest(&(target.consumer, &registration.community_id))?;
            let fresh = guard.try_mark_in_scope(&scope, &verified.native_event_id, 120).await.map_err(|_| Fault::Denied)?;
            if !fresh { return Err(Fault::Denied.into()); }
        }
        ...
        if inserted.rows_affected() != 1 { return Err(Fault::Denied.into()); }
    ...
    pub(crate) async fn authorize_command(&self, tx: &mut Tx, c: &Command, headers: &Headers<'_>, path: &str, body: &[u8], root: Option<&str>, purpose: &str) -> Result<(Registration, Claims)> {
        c.validate()?;
        let fingerprint = c.fingerprint()?;
        self.authorize(tx, headers, purpose, path, "POST", body, &Target {consumer:&c.consumer_id,intent:&c.intent_id,operation:&c.operation,resource:&c.resource,fingerprint:&fingerprint,root}).await
    }
    pub(crate) async fn replay(tx: &mut Tx, c: &Command) -> Result<Option<CommandResult>> {
        ...
        match row {
            None => Ok(None),
            Some(row) => {
                if row.try_get::<String,_>("request_sha256")? != c.fingerprint()? { return Err(Fault::Conflict.into()); }
                let mut result: CommandResult = row.try_get::<sqlx::types::Json<CommandResult>,_>("result")?.0;
                result.replayed = true; Ok(Some(result))
            }
        }
    }
    pub(crate) async fn remember<T: Serialize>(tx: &mut Tx, c: &Command, operation_id: String, execution: Execution, revision: u64, result: T) -> Result<CommandResult> {
        let response = CommandResult {
            receipt: Receipt {contract:CONTRACT.into(),profile:PROFILE.into(),consumer_id:c.consumer_id.clone(),intent_id:c.intent_id.clone(),operation_id,request_sha256:c.fingerprint()?,admission:Admission::Accepted,execution,revision,effect_refs:vec![]},
            result: serde_json::to_value(result).map_err(|_| Fault::Invalid)?, replayed:false,
        };
    ...
    pub(crate) fn decode<T: DeserializeOwned>(bytes: &[u8]) -> Result<T> { Ok(llull_buzz_wire::parse(bytes)?) }
}
...
struct TransactionReplayGuard<'c> { connection: Mutex<&'c mut PgConnection> }
impl Nip98ReplayGuard for TransactionReplayGuard<'_> {
    fn try_mark_in_scope<'a>(&'a self, scope: &'a str, event_id: &'a nostr::EventId, ttl_secs: u64) -> Pin<Box<dyn Future<Output=std::result::Result<bool,buzz_auth::error::AuthError>> + Send + 'a>> {
        Box::pin(async move {
            let ttl = ttl_secs.clamp(buzz_auth::DEFAULT_REPLAY_TTL_SECS,buzz_auth::MAX_REPLAY_TTL_SECS) as i64;
